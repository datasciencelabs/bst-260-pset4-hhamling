...
population <- population |> as_tibble() |> row_to_names(1, remove_row = TRUE, remove_rows_above = TRUE) |> rename(state_name = NAME, `2020` = POP_2020, `2021` = POP_2021) |> select(-4) |> arrange(state_name) |>  mutate(across(c("2020","2021"), as.numeric)) |> 
  mutate(state = case_when(
    ...
    state_name == "Puerto Rico" ~ "PR")) |> pivot_longer(c(`2020`,`2021`), names_to = "year", values_to = "population")
...
population |> ggplot(aes(x = reorder(state_name, population), y = population)) + geom_col() + facet_wrap(~ year) + coord_flip() + labs(x = "Population", y = "State", title = "State populations by year") 
```
...
url <- "https://github.com/datasciencelabs/2025/raw/refs/heads/main/data/regions.json"
json_dat <- fromJSON(url)

regions1 <- fromJSON(url) |> separate_longer_delim(states, delim = ",") #doesn't unnest the list

regions <- map_dfr(fromJSON(url, simplifyVector = FALSE), function(region){
  tibble(region_name = region$name,
         state_name = region$states)
}
  )

regions <- fromJSON(url) |> unnest(states) #not sure why we can't use unnest
```
...
cases_raw <- request(api) |>
  ...
  req_perform() |> resp_body_json(simplifyVector = TRUE) |> select("state", "end_date", "tot_cases") |> mutate(end_date = ymd_hms(end_date)) |> rename(cases = tot_cases, date = end_date)
```
...
```{r}
library(knitr)
cases |> mutate(year = year(date), month = month(date, label = TRUE), cases = as.numeric(cases)) |> select(state, year, month, cases) |> filter(year == 2020 | year == 2021) |> group_by(year, month) |> summarize(total_cases = sum(cases,na.rm = TRUE)) |> kable(col.names = c("Year", "Month", "Total Cases"))
```
...
deaths <- request(deaths_url) |>
  ...
  mutate(date = ymd_hms(date), deaths = as.numeric(deaths))
```
...
```{r}
deaths |> filter(state!= "United States") |> group_by(state) |> summarize(total_deaths = sum(deaths, na.rm = TRUE)) |> arrange(desc(total_deaths)) |> slice_head(n=10) |> 
