...
regions <- fromJSON(url, simplifyDataFrame = FALSE) |> map_df(as.data.frame) |> mutate(region = as.numeric(region), region_name = as.character(region_name), states = as.character(states)) |> separate_rows(states, sep = ", ") |> rename(state_name = states) |> mutate(state_name = str_remove_all(state_name, 'c\\("|"\\)|"')) |> mutate(region_name = ifelse(region_name == "New York and New Jersey, Puerto Rico, Virgin Islands", "NY NJ PR VI", region_name))
```
...
print(nrow(cases_raw)) #1000 rows
```
...
cases <- request(api) |>
  ...
  mutate(end_date = ymd_hms(end_date), new_cases = as.numeric(new_cases)) |> 
...
```{r}
(cases |> filter(year(date) == 2020 | year(date) == 2021) |>
   inner_join(population_joined, by = "state") |> 
   group_by(state, date, region_name) |> 
   summarize(case_per_100k = (cases/population)*100000) |> 
   ggplot(aes(x = date, y = case_per_100k, color = state)) +
...
```{r}
library(knitr)
cases |> mutate(year = year(date), month = month(date, label = TRUE), cases = as.numeric(cases)) |> select(state, year, month, cases) |> filter(year == 2020 | year == 2021) |> group_by(year, month) |> summarize(total_cases = sum(cases,na.rm = TRUE)) |> 
