...
population <- population |> as_tibble() |> 
  ...
  select(-4) |> arrange(state_name) |>  
  mutate(across(c("2020","2021"), as.numeric)) |> 
...
population |> ggplot(aes(x = reorder(state_name, population), y = population)) + 
  geom_col() + facet_wrap(~ year) + coord_flip() + 
...
regions <- fromJSON(url, simplifyDataFrame = FALSE) |> map_df(as.data.frame) |>
  mutate(region = as.numeric(region), 
         region_name = as.character(region_name), 
         states = as.character(states)) |> separate_rows(states, sep = ", ") |> 
  rename(state_name = states) |> 
  mutate(state_name = str_remove_all(state_name, 'c\\("|"\\)|"')) |> 
  mutate(
  region_name = ifelse(
...
```{r}
library(knitr)
cases |> mutate(year = year(date), month = month(date, label = TRUE), 
            cases = as.numeric(cases)) |> select(state, year, month, cases) |>
  filter(year == 2020 | year == 2021) |> group_by(year, month) |> 
...
```{r}
deaths |> filter(state!= "United States" & state!= "New York City") |> 
  group_by(state) |> summarize(covid_deaths = sum(deaths, na.rm = TRUE)) |> 
  arrange(desc(covid_deaths)) |> head(n=10) |> 
